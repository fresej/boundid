...
boundid_test <- function(Data, X_var, threshold, floor = TRUE, treatment, time, ID,t, first_period, S = NULL, more_info = F, stag = FALSE) {
  ...
  if (stag) {
    ...
    means_A_0 <- sapply(X_A_0_list, function(X) mean(X$X_var_alt, na.rm = TRUE)) 
    means_A_1 <- sapply(X_A_1_list, function(X) mean(X$X_var_alt, na.rm = TRUE)) 
